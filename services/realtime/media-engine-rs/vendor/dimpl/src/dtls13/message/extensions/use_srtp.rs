...
impl UseSrtpExtension {
    ...
    /// caller's preference order and dropping duplicates. DTLS 1.3 twin of the
    /// DTLS 1.2 helper: the extension carries at most three profiles, so a
    /// longer configured list keeps the first three instead of panicking.
    pub fn from_profiles(profiles: &[SrtpProfile]) -> Self {
        let mut list = ArrayVec::<SrtpProfileId, 3>::new();
