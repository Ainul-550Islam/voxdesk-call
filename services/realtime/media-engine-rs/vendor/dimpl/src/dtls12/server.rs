...
impl State {
    ...
    fn await_client_hello(self, server: &mut Server) -> Result<Self, InternalError> {
        ...
        // Select the SRTP profile this endpoint is configured to accept
        // (Config::srtp_profiles), in ITS preference order. The default list is
        // dimpl's AES256GCM, AES128GCM, SHA1; the vendored patch lets a caller
        // pin a narrower set so a profile the caller cannot carry is refused
        // instead of negotiated.
        if let Some(profiles) = client_srtp_profiles {
            let accepted: Vec<SrtpProfile> = server.engine.config().srtp_profiles().to_vec();
