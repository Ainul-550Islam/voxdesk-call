...
impl ClientHello {
    ...
    pub fn with_extensions(mut self, buf: &mut Buf, config: &Config) -> Self {
        ...
        // Add use_srtp extension for DTLS-SRTP support. Vendored patch: the
        // offer list is THIS endpoint's configured set (Config::srtp_profiles),
        // not dimpl's hardcoded GCM-first list, so a client can be pinned to
        // exactly the profiles the peer is permitted to accept.
