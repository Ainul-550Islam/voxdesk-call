...
impl UseSrtpExtension {
    ...
    /// Build a USE_SRTP offer from a configured profile list, keeping the
    /// caller's preference order and dropping duplicates.
    ///
    /// RFC 5764 §4.1.2 caps the extension at the three profiles in
    /// [`SrtpProfileId::supported`]; a longer configured list keeps the first
    /// three rather than panicking inside a handshake.
    pub fn from_profiles(profiles: &[SrtpProfile]) -> Self {
        let mut list = SrtpProfileVec::new();
        ...
        UseSrtpExtension::new(list, ArrayVec::new())
    }

    /// Create a default UseSrtpExtension with standard profiles.
    /// (Kept for API completeness; the VoxDesk patch routes offers
    /// through `Config::srtp_profiles` instead.)
    #[allow(dead_code)]
    pub fn default() -> Self {
