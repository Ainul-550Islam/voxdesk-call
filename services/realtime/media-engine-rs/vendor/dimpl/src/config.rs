...
impl Config {
    ...
    /// Allowed DTLS-SRTP protection profiles, most preferred first.
    ///
    /// The list is used in BOTH directions: it is what a client endpoint offers
    /// in its `use_srtp` extension, and it is the set a server endpoint is
    /// allowed to select from when answering a client's offered profiles.
    ///
    /// When no filter is set via the builder's `srtp_profiles` method, all
    /// profiles in [`SrtpProfile::ALL`] are used. An explicitly empty list is
    /// treated the same way: RFC 5764 §4.1.2 fixes a minimum of one profile in
    /// the extension, so an empty allow-list could never negotiate and is not a
    /// meaningful configuration.
    ///
    /// This accessor is the one upstream-facing knob added by this workspace's
    /// vendored-dimpl patch (see the `[patch.crates-io]` note in the workspace
    /// `Cargo.toml` and PROMPT2-DESIGN.md Amendment (d)): WebRTC interop needs
    /// dimpl to negotiate only the profiles the engine's own SRTP path can
    /// actually carry, instead of dimpl's hardcoded GCM-first set.
    pub fn srtp_profiles(&self) -> &[SrtpProfile] {
        match &self.srtp_profiles {
...
impl ConfigBuilder {
    ...
    /// Set the allowed DTLS-SRTP protection profiles, most preferred first.
    ///
    /// Only profiles present in this list are offered (client role) and
    /// selected (server role). Order determines preference.
    ///
    /// By default all profiles in [`SrtpProfile::ALL`] are used.
    pub fn srtp_profiles(mut self, profiles: &[SrtpProfile]) -> Self {
        self.srtp_profiles = Some(profiles.to_vec());
        self
    }
